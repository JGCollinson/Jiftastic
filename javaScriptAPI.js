 var topics = ["Jifs", "Star Wars", "Simpsons", "Star Trek", "Futurama"];
    ...
    function jifMethod() {
      $("button").on("click", function () {
        ...
        var queryURL = "https://api.giphy.com/v1/gifs/search?q=" + buttonText + "&api_key=DxDENWDds1OGZtB9v5uLLLB1JNoPyPFy&limit=12";
        $.ajax({
            url: queryURL,
            method: "GET"
          })
          .done(function (response) {
            ...
            for (var i = 0; i < results.length; i++) {
              ...
              jifObject.attr('src', results[i].images.original_off.url);
              jifObject.attr('image', results[i].images.original_off.url);
              jifObject.attr('jif', results[i].images.fixed_height.url);
              jifObject.attr('toggle', results[i].images.original_off.url);
